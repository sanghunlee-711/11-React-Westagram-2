...
class LeftSection extends React.Component{
  constructor(props){
    ...
    add = () =>{
      let texts = this.state.comments;
      texts.push({text:'Wecode ' +this.state.inputText})

      this.setState({comments: texts, inputText : "", })
    }

    id =()=>{
      this.setState({id: "wecode"})
    }
      ...
      <header  className="feed_header">
       <div className="feed_header_container">  
          <div className="feed_header_gui_container">
           ...
           <div className="feed_header_id_text">canon_mj</div>
         </div>
  ...
  <div className="like_share_comment_container">
    ...
    <div className="comment_section">
      <div className="comment_section_my_text"><a href="">canon_mj  </a><span>위워크에서 진행한 베이킹 클래스<span>...</span></span><span><button>더보기</button></span></div>
      <ul className="comment_section_user_text">
        <li><a href="#">ID</a>&nbsp;사랑해요!! </li>
        {this.state.comments.map((v) => (<li> {this.state.id} {v.text}</li>))}
      </ul>
    </div>
